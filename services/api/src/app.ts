...
export function createApp(store: DemoStore = demoStore, options: AppOptions = {}) {
  ...
  app.use('/api', (request, _response, next) => {
    ...
    const livePath = /^(?:\/health(?:\/|$)|\/auth\/(?:challenge|verify|session)$|\/trust-model$|\/runtime\/paid-capability$|\/training\/(?:catalog|agents\/[^/]+\/reports)$|\/arena\/leaderboard$|\/dashboard\/pg$|\/leaderboard\/pg$|\/agents\/pg(?:\/.*)?|\/agents\/[^/]+\/(?:autopilot|api-keys|work-queue)(?:\/.*)?|\/agent-runs\/pg(?:\/.*)?|\/tasks\/pg(?:\/.*)?|\/templates\/pg(?:\/.*)?|\/deliverables\/pg(?:\/.*)?|\/disputes\/pg(?:\/.*)?)$/.test(request.path);
    if (!livePath) {
