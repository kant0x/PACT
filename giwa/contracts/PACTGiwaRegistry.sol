...
contract PACTGiwaRegistry {
    ...
    modifier onlyIssuer() { if (!authorizedIssuers[msg.sender]) revert Unauthorized(); _; }
    ...
    function recordTask(address agent, bytes32 taskId, bool successful, uint256 points, bytes32 evidenceHash) external onlyIssuer {
        ...
        taskReceipts[taskId] = TaskReceipt(agent, msg.sender, evidenceHash, uint64(block.timestamp), successful, points);
    ...
    function getAgent(address agent) external view returns (AgentProfile memory) { return profiles[agent]; }
    function getTaskReceipt(bytes32 taskId) external view returns (TaskReceipt memory) { return taskReceipts[taskId]; }
}
